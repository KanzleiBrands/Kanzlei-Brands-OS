...
const AI_MODES = ["generate", "rewrite", "shorten", "lengthen", "tone", "translate", "hashtags"] as const;
...
function buildPrompt(
  ...
  switch (mode) {
    case "generate":
      return `Schreibe einen neuen Social-Media-Beitragstext auf Deutsch für ${platformNote} zum Thema: "${instruction}". Gib NUR den fertigen Text zurück, ohne Anführungszeichen oder Erklärungen.`;
    case "rewrite":
      return `Schreibe den folgenden Social-Media-Beitragstext für ${platformNote} um - gleicher Sinn, neue Formulierung. Gib NUR den neuen Text zurück, ohne Anführungszeichen oder Erklärungen.\n\nText:\n${caption}`;
    case "shorten":
      return `Kürze den folgenden Social-Media-Beitragstext für ${platformNote} deutlich, ohne die Kernaussage zu verlieren. Gib NUR den gekürzten Text zurück, ohne Anführungszeichen oder Erklärungen.\n\nText:\n${caption}`;
    case "lengthen":
      return `Erweitere den folgenden Social-Media-Beitragstext für ${platformNote} um mehr Details, ohne den Stil zu verändern. Gib NUR den erweiterten Text zurück, ohne Anführungszeichen oder Erklärungen.\n\nText:\n${caption}`;
    case "tone":
      return `Ändere den Ton des folgenden Social-Media-Beitragstexts für ${platformNote} zu "${instruction}". Gib NUR den neuen Text zurück, ohne Anführungszeichen oder Erklärungen.\n\nText:\n${caption}`;
    case "translate":
      return `Übersetze den folgenden Social-Media-Beitragstext ins ${instruction}, sinngemäß und natürlich klingend. Gib NUR die Übersetzung zurück, ohne Anführungszeichen oder Erklärungen.\n\nText:\n${caption}`;
    case "hashtags":
...
export async function assistSocialCaption(input: {
  ...
  if ((input.mode === "generate" || input.mode === "tone" || input.mode === "translate") && !input.instruction?.trim()) {
    return { error: "Bitte zuerst ein Thema, einen Ton bzw. eine Sprache angeben." };
  }
